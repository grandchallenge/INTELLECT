...
class RsiCccWp00M3Tests(unittest.TestCase):
    ...
    def test_artifact_is_exact_generated_replay(self) -> None:
        ...
        self.assertEqual(
            result.transcript_chain_digest,
            "295b815b1a14c4453da824d28c7e85309799678b737f274c795ebddd0704a008",
        )
        self.assertEqual(
            result.independent_certificate_chain_digest,
    ...
    def test_bounded_proposer_materially_generates_mixed_sequence(self) -> None:
        artifact = self.load_artifact()
        proposal_digests = [event["proposal_digest"] for event in artifact["events"]]
        self.assertEqual(
            proposal_digests,
            [
        ...
        self.assertEqual(len(set(proposal_digests)), 5)
    ...
    def test_proposer_private_identity_does_not_change_independent_certificate(self) -> None:
        ...
        envelope_b = clone_with_private_identity(
            envelope_a,
            ProposerPrivateState("different-proposer", "private-state-B", 1001),
        )
    ...
    def test_forged_certificate_field_is_rejected_by_frozen_checker(self) -> None:
        old, envelope = first_envelope()
        mutated = deepcopy(envelope)
        mutated["proposal"]["certificate"] = {"accepted": True, "code": "accepted"}
        recommit(mutated)
        result = evaluate_committed_envelope(old, mutated)
        self.assertFalse(result.accepted)
        self.assertEqual(result.code, "malformed")

    def test_candidate_defined_metric_is_rejected(self) -> None:
        old, envelope = first_envelope()
        mutated = deepcopy(envelope)
        mutated["proposal"]["metric"] = {"name": "proposer-defined", "score": 0}
        recommit(mutated)
        result = evaluate_committed_envelope(old, mutated)
        self.assertFalse(result.accepted)
        self.assertEqual(result.code, "malformed")

    def test_stale_predecessor_is_rejected_after_valid_transcript_commit(self) -> None:
        old, envelope0 = first_envelope()
        first = evaluate_committed_envelope(old, envelope0)
        self.assertTrue(first.accepted)
        current = first.next_state
        envelope1 = generate_committed_envelope(
            public_state_view(current),
            ProposerPrivateState("bounded-search-proposer-A", "m3-prefix-cost-v1", 1),
        )
    ...
    def test_context_and_kernel_substitution_are_rejected(self) -> None:
        ...
        for field, value, code in (
            ...
            mutated["proposal"]["candidate_digest"] = sha256_json(
                mutated["proposal"]["candidate"]
            )
            recommit(mutated)
            result = evaluate_committed_envelope(old, mutated)
            self.assertFalse(result.accepted)
    ...
    def test_resource_limit_and_policy_smuggling_fail_closed(self) -> None:
        old, envelope = first_envelope()
        mutated = deepcopy(envelope)
        mutated["proposal"]["max_declared_cost"] = 999999999
        recommit(mutated)
        result = evaluate_committed_envelope(old, mutated)
        self.assertFalse(result.accepted)
        self.assertEqual(result.code, "malformed")
        ...
        result = evaluate_committed_envelope(old, top_level)
        self.assertFalse(result.accepted)
        self.assertEqual(result.code, "malformed_envelope")

    def test_transcript_and_objective_substitution_fail_before_admission(self) -> None:
        old, envelope = first_envelope()
        transcript_mutation = deepcopy(envelope)
        transcript_mutation["private_state_digest"] = "00" * 32
        result = evaluate_committed_envelope(old, transcript_mutation)
        self.assertFalse(result.accepted)
        self.assertEqual(result.code, "transcript_mismatch")
        ...
        result = evaluate_committed_envelope(old, objective_mutation)
        self.assertFalse(result.accepted)
        self.assertEqual(result.code, "objective_mismatch")
    ...
    def test_frozen_checker_and_formal_object_are_byte_stable(self) -> None:
        self.assertEqual(
            git_blob_sha(CHECKER_PATH.read_bytes()),
            "e4ad8cef6fdf514c2aad0dbac372b3a42f5e6dbb",
        )
        self.assertEqual(
            git_blob_sha(FORMAL_OBJECT_PATH.read_bytes()),
            "8baee8d3a1f50a4d527afdc29747a0fa563451bb",
        )

    def test_manifest_binds_m3_and_remains_non_promotable(self) -> None:
        manifest = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
        self.assertFalse(manifest["promotion_ready"])
        m3 = manifest["mechanization"]["m3_tranche"]
        self.assertEqual(m3["issue"], 108)
        self.assertEqual(m3["objective_digest"], OBJECTIVE_DIGEST)
        self.assertEqual(
            m3["transcript_chain_digest"],
            "295b815b1a14c4453da824d28c7e85309799678b737f274c795ebddd0704a008",
        )
        self.assertEqual(
            m3["independent_certificate_chain_digest"],
            "7120c0be21626ab9a29b61a71f5a7837ea4c1cb163be54592d3f2f20d89411d8",
        )
