...
from .trove_curata_duplicate_identity import FIXTURE_ID, SCHEMA_VERSION, PREDECESSOR, PROVIDER_LOCK, PERMUTATION_TABLE_SHA256, ROUTES, EDGE_BASES, RECORD_CLASSES, ORIGIN_KINDS, REQUIRED_CASE_CLASSES, FALSE_CLAIMS, TroveCurataDuplicateError, require, canonical_json_bytes, sha256_bytes, content_id, safe_relative_path, provider_versions
...
def load_manifest(path: str | Path) -> dict[str, Any]:
    ...
    require(set(manifest) == {'schema_version', 'fixture_id', 'predecessor', 'authority', 'records_file', 'predecessor_manifest', 'configuration', 'cases', 'expected_components'}, 'manifest field set drift')
    ...
    expected_configuration = {'normalization': 'unicode_nfc_line_endings_whitespace_lower_v1', 'tokenizer': 'unicode_word_v1', 'shingle_size': 3, 'threshold': '0.720000', 'provider': {'name': 'datasketch', 'version': '2.0.0', 'numpy_version': '2.5.1', 'scipy_version': '1.18.0', 'scheme': 'affine32', 'num_perm': 128, 'seed': 17, 'permutation_table_sha256': PERMUTATION_TABLE_SHA256}, 'baseline': {'name': 'grand_intellect_jaccard', 'version': SCHEMA_VERSION}, 'conservative_record_classes': ['code_math', 'short_text']}
    ...
    classes: set[str] = set()
    for case in cases:
        require(isinstance(case, dict) and set(case) == {'case_id', 'case_class', 'left_record_id', 'right_record_id', 'expected_exact_byte', 'expected_normalized_text', 'expected_provider_threshold', 'expected_baseline_threshold', 'expected_admitted_edge', 'expected_disagreement', 'expected_edge_basis', 'expected_route'}, 'case field set drift')
        ...
        require(isinstance(left, str) and isinstance(right, str) and left and right, 'record identities required')
        require(left != right, 'self-pairs are prohibited')
        pair = tuple(sorted((left, right)))
        require(pair not in pair_ids, 'duplicate unordered pair identity')
        pair_ids.add(pair)
        for key in ('expected_exact_byte', 'expected_normalized_text', 'expected_provider_threshold', 'expected_baseline_threshold', 'expected_admitted_edge', 'expected_disagreement'):
    ...
    for component in components:
        require(isinstance(component, list) and len(component) >= 2, 'component requires at least two members')
        require(component == sorted(component) and len(component) == len(set(component)), 'component member order or uniqueness drift')
...
def load_records(path: str | Path) -> dict[str, Any]:
    ...
    require(set(payload) == {'schema_version', 'fixture_id', 'records'}, 'records field set drift')
    require(payload['schema_version'] == SCHEMA_VERSION and payload['fixture_id'] == FIXTURE_ID, 'records identity drift')
    require(isinstance(payload['records'], list) and payload['records'], 'retained records required')
    ids: set[str] = set()
    for record in payload['records']:
        require(isinstance(record, dict) and set(record) == {'record_id', 'record_class', 'language', 'origin', 'text'}, 'record field set drift')
        ...
        if origin['kind'] == 'tc003_baseline_output':
            require(isinstance(origin['case_id'], str) and origin['case_id'], 'TC003 origin case required')
        else:
            require(origin['case_id'] is None, 'fixture-retained origin must not name a predecessor case')
